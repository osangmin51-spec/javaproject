...
- 10개씩 총 10페이지 페이지네이션
- 3초 간격 실시간 모의 시세 갱신
- 검색 또는 상위 100 목록 클릭 기반 종목 선택 주문
- 시장가/지정가 모의 매수·매도 주문
- 조건 미달 지정가 주문 대기 처리
...
주문 폼은 직접 종목코드를 외워서 입력하는 방식이 아니라, 먼저 종목을 선택한 뒤 수량과 주문유형을 입력하는 방식입니다.

- 검색창에서 종목코드나 회사명을 검색해 선택할 수 있습니다.
- `S&P 500 등락률 상위 100` 표에서 행 또는 `선택` 버튼을 누르면 해당 종목이 주문 대상이 됩니다.
- 선택한 종목의 현재가, 등락률, 섹터, 보유수량, 평균단가, 갱신시각을 주문창에서 확인할 수 있습니다.
- 수량과 지정가를 바꾸면 예상 주문금액이 즉시 갱신됩니다.
...
상단의 `실시간 시작` 버튼을 누르면 3초마다 `/api/sim/tick`이 호출되어 모의 시세, 계좌 지표, 수익률 분석, 선택 종목 상세 정보가 자동으로 갱신됩니다. 대기 중인 지정가 주문이 조건을 만족하면 이 갱신 흐름에서 자동 체결됩니다.

### S&P 500 등락률 상위 100

S&P 500 구성종목 중 모의 등락률이 높은 순서로 상위 100개를 보여줍니다. 한 페이지에 10개씩 표시되며, 1페이지는 1~10위, 2페이지는 11~20위입니다. 표의 종목 행을 클릭하면 주문창의 선택 종목이 바로 바뀝니다.
