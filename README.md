...
실제 시세 연동을 확장한다면 한국투자증권 KIS Open API를 사용하는 구조가 적합합니다. 현재 프로젝트는 API Key 없이 실행 가능하도록 내부 모의 증권사 서버를 사용하며, 실제 연동 시에는 `MockBrokerServer` 부분을 KIS WebSocket 수신 모듈로 교체하는 방식으로 확장할 수 있습니다.

실제 증권사 연동에는 API Key, Secret, 계좌 정보, 모의투자 신청 정보가 필요합니다.
