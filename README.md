...
구현 후 코드를 다시 점검하면서 실제 거래 시스템 관점에서 위험한 부분을 하나 더 발견했습니다.

### 발견한 문제

대기 중인 지정가 주문은 처음 주문할 때만 예수금과 보유 수량을 검사했습니다. 그런데 주문이 `PENDING` 상태로 남아 있는 동안 사용자가 다른 주문으로 예수금을 쓰거나 보유 주식을 팔아버릴 수 있습니다.

이 상태에서 나중에 `/api/sim/tick`으로 가격이 변해 지정가 조건이 충족되면, 기존 구조에서는 현재 예수금이나 보유 수량을 다시 확인하지 않고 자동 체결될 위험이 있었습니다.

### 개선한 방식

`LimitOrderExecutor`가 실제 체결 직전에 `OrderValidator`를 다시 실행하도록 수정했습니다.

이제 대기 지정가 주문이 자동 체결되기 직전에 아래 조건을 다시 검사합니다.

- 종목코드가 유효한지
- 수량이 정상인지
- 매수 주문이면 현재 예수금이 충분한지
- 매도 주문이면 현재 보유 수량이 충분한지

검증에 실패하면 주문은 체결되지 않고 `REJECTED` 상태가 됩니다. 이 흐름은 실제 주문 시스템의 "체결 직전 위험 검사"에 더 가깝습니다.

또한 `시장 변동 시뮬레이션` 버튼을 눌렀을 때 자동 체결된 지정가 주문 수를 화면 메시지로 보여주도록 개선했습니다.
...
- 10개씩 총 10페이지 페이지네이션
- 3초 간격 실시간 모의 시세 갱신
- 종목 탐색, 상세 정보, 매수/매도 티켓을 분리한 트레이딩 화면
- 검색 또는 상위 100 목록 클릭 기반 종목 상세 확인
- 선택 종목 기준 실시간 보유 손익 및 주문 예상금액 표시
- 선택 종목 외부 현재가 연동(`stockprices.dev` 우선, Stooq CSV fallback)
- Java 콘솔 미니프로젝트 참고 기능 웹 이식: 다음날 진행, 아이템 상점, 오늘의 운세, 가격예측권, 자유게시판/댓글
- 시장가/지정가 모의 매수·매도 주문
- 조건 미달 지정가 주문 대기 처리
...
화면은 `종목 탐색 -> 종목 상세 -> 매수/매도 -> 실시간 손익 확인` 흐름으로 구성됩니다.

- 왼쪽: S&P 500 등락률 상위 100 종목 탐색, 종목 검색, 페이지 이동
- 가운데: 선택 종목의 현재가, 등락률, 섹터, 실시간 추세 차트, 보유 포지션 손익
- 오른쪽: 선택 종목 기준 매수/매도 티켓, 예상 주문금액, 거래 후 예상 보유수량
- 하단/보조 영역: 최근 주문, 내 보유 종목, 관심종목

종목코드를 직접 외워 입력하지 않아도 됩니다. 검색창에서 종목코드나 회사명을 찾거나, 상위 100 목록의 행을 클릭하면 해당 종목이 상세 화면과 주문 티켓에 바로 연결됩니다.
...
GitHub의 Java 콘솔 미니프로젝트 구조를 웹 기능으로 옮겼습니다. 원본은 `MemberController`, `StockController`, `BulletinController`, `ItemController` 중심의 콘솔 메뉴였고, 이 프로젝트에서는 기존 웹 계좌/주문 시스템과 연결했습니다.

- `다음날 진행`: 원본의 다음날 메뉴처럼 모의 시세를 한 번 변동시키고 대기 지정가 주문을 검사합니다.
- `아이템 상점`: `오늘의 운세`, `가격예측권` 아이템을 구매하거나 사용할 수 있습니다.
- `오늘의 운세`: 무작위 운세 메시지를 보여줍니다.
- `가격예측권`: 선택 종목 또는 무작위 종목의 다음 변동 힌트를 보여줍니다.
- `자유 게시판`: 게시글 작성과 댓글 작성이 가능합니다.

원본 콘솔 프로젝트의 회원가입/로그인 흐름은 현재 웹앱의 demo 계좌 구조와 겹치기 때문에 별도 로그인 화면으로 복제하지 않고, demo 사용자 기준 기능으로 통합했습니다.

### 실시간 모의 시세

페이지가 열리면 실시간 모의 갱신이 자동으로 시작됩니다. 상단의 `실시간 중지` 버튼으로 멈출 수 있고, 다시 `실시간 시작`을 누르면 3초마다 `/api/sim/tick`이 호출됩니다.

갱신 때마다 모의 시세, 계좌 지표, 선택 종목 상세 정보, 실시간 손익, 추세 차트가 함께 바뀝니다. 대기 중인 지정가 주문이 조건을 만족하면 이 갱신 흐름에서 자동 체결됩니다.

선택한 종목은 별도로 `/api/external/quote?symbol=AAPL`을 통해 외부 현재가를 가져옵니다. 서버는 먼저 `stockprices.dev` JSON API를 시도하고, 실패하면 Stooq CSV 시세를 시도합니다. 응답에는 `source`와 `sourceUrl`을 함께 내려주며, 두 소스 모두 실패하면 기존 모의 시세를 그대로 사용하므로 네트워크가 불안정해도 앱이 멈추지 않습니다.

외부 데이터는 무료/공개 소스 특성상 지연되거나 일시적으로 실패할 수 있습니다. 토스증권, 나스닥, 거래소의 공식 실시간 시세가 아니며, 이 프로젝트에서는 실제 투자용이 아니라 모의투자 화면과 데이터 연동 구조를 보여주는 용도로 사용합니다.

### S&P 500 등락률 상위 100

S&P 500 구성종목 중 모의 등락률이 높은 순서로 상위 100개를 보여줍니다. 한 페이지에 10개씩 표시되며, 1페이지는 1~10위, 2페이지는 11~20위입니다. 표의 종목 행을 클릭하면 가운데 상세 화면과 오른쪽 주문 티켓이 바로 바뀝니다.
...
| `GET` | `/api/external/quote` | 선택 종목 외부 현재가 조회 및 내부 시세 반영 |
...
| `GET` | `/api/mini/status` | 미니프로젝트 진행일, 아이템, 게시판 요약 |
| `GET` | `/api/mini/items` | 아이템 상점 목록 |
| `GET` | `/api/mini/board` | 자유게시판 게시글/댓글 목록 |
| `POST` | `/api/mini/day/next` | 다음날 진행 및 시세 변동 |
| `POST` | `/api/mini/items/buy` | 아이템 구매 |
| `POST` | `/api/mini/items/use` | 아이템 사용 |
| `POST` | `/api/mini/board/write` | 게시글 작성 |
| `POST` | `/api/mini/board/comment` | 댓글 작성 |
...
4. 지정가 조건을 만족한 주문에 대해 체결 직전 예수금/보유 수량 재검증
5. 검증 통과 시 체결, 실패 시 거절
6. 변경된 계좌와 주문 내역 파일 저장
