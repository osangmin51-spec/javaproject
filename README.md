...
선택한 종목은 별도로 `/api/external/quote?symbol=AAPL`을 통해 외부 현재가를 가져옵니다. 서버는 먼저 `stockprices.dev` JSON API를 시도하고, 실패하면 Stooq CSV 시세를 시도합니다. 두 소스 모두 실패하면 기존 모의 시세를 그대로 사용하므로 네트워크가 불안정해도 앱이 멈추지 않습니다.

외부 데이터는 무료/공개 소스 특성상 지연되거나 일시적으로 실패할 수 있습니다. 이 프로젝트에서는 실제 투자용이 아니라 모의투자 화면과 데이터 연동 구조를 보여주는 용도로 사용합니다.
