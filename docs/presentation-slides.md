...
- 보유/즐겨찾기/기록 탭
- 발표자료에는 실제 웹사이트 캡처 3장(시장 시세, 종목 상세/주문, 보유 손익)을 배치
...
1. 브라우저가 `/api/stock/buy` 호출
2. `MiniProject`가 가격 × 수량과 잔액 확인
3. `shares` 수량과 매입금액 갱신
4. `trade_logs`에 거래 기록 추가
5. `/api/state`로 평가금액과 손익 다시 표시
