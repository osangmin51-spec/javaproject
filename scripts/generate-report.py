...
paragraph("교수님이 프로젝트를 처음 볼 때 가장 먼저 이해해야 하는 부분은 사용자의 화면 조작이 Java 서버, 외부 시세 API, MySQL 저장소로 어떻게 이어지는지이다. 이 프로젝트는 프론트엔드 프레임워크 없이 Java 서버가 HTML/CSS/JavaScript 화면을 직접 내려주고, 브라우저는 필요한 순간 API를 호출해 상태를 다시 받아오는 구조다. 화면에서는 서버 동기화 사이에도 1초 단위 표시용 가격 틱을 만들어 그래프가 멈춰 보이지 않게 했다.")
...
add_bullets([
    ...
    "입력 보호: 매수/매도 수량 입력 중에는 주문 영역과 보유 테이블을 다시 그리지 않아 입력값이 끊기지 않음",
])
...
add_table(["데이터", "처리 방식"], [
    ...
    ["실시간성", "전체 종목 매초 조회 대신 상위 목록 중심 KIS 동기화와 브라우저 1초 표시용 가격 틱 사용"],
    ["소켓", "모의 증권사 서버의 가격 Tick 구독 구조"],
    ["사용자 데이터", "MySQL 우선 저장, 연결 실패 시 TSV 파일 저장"],
    ["가격 그래프", "서버가 보관한 최근 가격 히스토리에 화면 표시용 1초 틱을 더해 변화가 보이게 표시"],
], widths=[2.0, 5.2])
...
paragraph("최종 결과물은 Java로 직접 만든 모의주식투자 웹앱이다. 한국투자증권 KIS API로 시세와 거래량을 가져오고, 모의 계좌의 보유주식과 거래기록은 MySQL을 우선 사용해 저장한다. MySQL 연결이 불가능한 환경에서는 TSV 파일 저장소로 전환된다. 사용자는 종목을 검색하거나 즐겨찾기하고, 상세 화면에서 가격 그래프를 확인한 뒤 매수/매도를 진행할 수 있다. 화면에서는 1초 단위 표시용 가격 변동을 보여주되, 매수/매도 입력창은 불필요하게 다시 그리지 않아 주문 흐름을 방해하지 않는다.")
